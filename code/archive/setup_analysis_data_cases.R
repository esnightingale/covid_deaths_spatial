...
setup_analysis_data_cases <- function(alldata, 
                                start = ymd("2020-03-01"), 
                                end = ymd("2020-08-01")){

  
alldata <- alldata %>%
    filter(dos >= start & dos <= end)
...
alldata %>%
  group_by(lad19cd, wos) %>%
  count() %>%
  full_join(la_pops) %>%
  ungroup() %>%
  mutate(n = replace_na(n, 0),
         w = as.integer(lubridate::week(wos)),
         SIR_wk = n/la_wk_E,
...
dat <- d_agg_wk %>%
  ...
  dplyr::select(n, E, E_wk, w, wos, lad19cd, lad19nm, la_pop, geog, geography, area_km2, pop_dens,
...
saveRDS(dat, paste0(datadir,sprintf("Cases/dat_cases_%s_%s.rds",start,end)))
