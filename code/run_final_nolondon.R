...
# Replace all counts for London with NAs
dat$n[dat$geog == "London Borough"] <- NA
...
fit <- fit_mod(f_bym_geog)
saveRDS(fit, file = here("output","fit_nolondon.rds"))

## Draw posterior samples ##
sample <- inla.posterior.sample(fit, n = 1000)
saveRDS(sample, file = here("output","sample_nolondon.rds"))
