...
saveRDS(fits, file = here::here("output",sprintf("fits_alt_%s_%s.rds",measure, wave)))

# ## Draw posterior samples ##
samples <- lapply(fits, inla.posterior.sample,n = 1000)
saveRDS(samples, file =  here::here("output",sprintf("samples_alt_%s_%s.rds",measure, wave)))


## BYM spatial effect, la-dependent 
f_bym_la <- n ~ 
  IMD + prop_minority + #log(pop_dens) +
  f(la, model = "bym2",
    group = wk_since_first+1, 
...
fit_bym_la <- fit_mod(f_bym_la, dat, E = E_wk)
summary(fit_bym_la)
