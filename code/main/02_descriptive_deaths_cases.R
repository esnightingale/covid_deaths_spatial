...
source(here::here("code","utils","functions.R"))
source(here::here("code","utils","plot_functions.R"))
...
for (dat in seq_along(data.list)){
  # for (wave in 1:2){
    ...
    print(plot_la_ts(data.list[[dat]], wave, title1 = titles[1,dat], title2 = titles[2,dat]))
    dev.off()
  # }
}
...
png(here::here(figdir,"ts_geog.png"), height = 800, width = 1200, res = 150)
...
summary(ratio_la$CFR_obs)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.02935 0.11765 0.16667 0.22368 0.26087 2.00000 


png(here::here("figures","compare","map_obs_cfr_postP2.png"), height = 800, width = 1000, res = 150)
regions %>%
  full_join(ratio_la) %>%
  basic_map(fill = "CFR_obs") +
  labs(title = "Observed ratio of deaths to cases after expansion of Pillar 2 testing", subtitle =  "2020-05-18 - 2020-06-28", fill = "Ratio")
dev.off()
...
ratio_pre %>%
  ...
  summarise(n_d = sum(n_d, na.rm = T),
            n_c = sum(n_c, na.rm = T),
            CFR_obs = n_d/n_c,
            CFR_obs_rel = CFR_obs/scale) -> ratio_pre_la

summary(ratio_pre_la$CFR_obs)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.08344 0.20581 0.25135 0.25528 0.29948 0.60000 

png(here::here("figures","compare","map_obs_cfr_preP2.png"), height = 800, width = 1200, res = 150)
regions %>%
  full_join(ratio_pre_la) %>%
  basic_map(fill = "CFR_obs") +
  labs(title = "Observed deaths:cases before expansion of Pillar 2 testing", subtitle = "2020-01-04 - 2020-05-17", fill = "Ratio")
dev.off()
