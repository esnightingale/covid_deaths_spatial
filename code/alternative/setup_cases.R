...
linelist %>%
  rename(dor = date_lab_report,
         dos = date_specimen) %>%
  mutate_at(vars(dor, dos), lubridate::ymd) %>%
  filter(!is.na(age)) %>% #n = 9
  mutate(result_delay = as.integer(difftime(dor, dos, units = "days")),
         wor = lubridate::floor_date(dor, unit = "week"),
         age_group = as.character(cut(age, breaks = c(0,seq(10,90,10),120), right = FALSE))) %>%
  arrange(age) %>%
  dplyr::select(lad19cd, lad19nm, wor, dor, dos, result_delay, age_group, pillar) -> alldata
...
alldata <- filter(alldata, dor > ymd("2020-01-01"))

# Include cases between 2020-03-01 and 2020-08-30
# alldata <- filter(alldata, dor %within% interval(ymd("2020-03-01"),ymd("2020-08-30")))

saveRDS(alldata,paste0(datadir,"Cases/alldata.rds"))
